...
public class Bank {
    private final Map<String,User> usersCheking;
    private final Map<String,User> userSaving;

    public Bank(Map<String, User> userSaving, Map<String, User> usersCheking) {
        this.userSaving = userSaving;
        this.usersCheking = usersCheking;
    }
...
//    }
    ...
    public User registerUser() {
        ...
        userSaving.put(newUser.getCheckingAccountNumber(), newUser);
    ...
    public User authenticateUser(String accountNumber, String password) {
        if (!usersCheking.containsKey(accountNumber) && !userSaving.containsKey(accountNumber)) {
            return null;
        }
        User user = usersCheking.containsKey(accountNumber) ? usersCheking.get(accountNumber) : userSaving.get(accountNumber);
    ...
    public void deposit(Account account, double amount) {
        ...
        while (true)
        {
            ...
            if (amount <= 0) {
                ...
                User user = usersCheking.get(account.getAccountNumber());
    ...
    public void listAccounts() {
        if (usersCheking.isEmpty() &&  userSaving.isEmpty()) {
            System.out.println("No users registered yet.");
        } else {
            System.out.println("\n==== Bank Users & Accounts ====");
            for (User user : usersCheking.values()) {
